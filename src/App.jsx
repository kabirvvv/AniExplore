...
import {
  ...
  Zap, Layers, RefreshCw, AlertTriangle, WifiOff, Clock, BookOpen,
} from "lucide-react";
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-8">
            {animeList.map((anime) => (
              <AnimeCard
                key={anime.mal_id}
                anime={anime}
                isPriority={priorityIds.has(anime.mal_id)}
                onRead={setReadingAnime}
              />
            ))}
          </div>
        )}
            ...
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/5 backdrop-blur-xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.3em] mt-1">Powered by Groq</p>
                </div>
              </div>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white/5 px-8 py-4 rounded-[2rem] rounded-tl-none border border-white/10 flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">AI Processing...</span>
                  </div>
                </div>
              )}
              <div ref={chatEndRef} />
